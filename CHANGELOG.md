...
- Prebuilt standalone binaries for Windows, macOS (Apple Silicon), and macOS (Intel) — both GUI (`phi-anonymize-gui`) and CLI (`phi-anonymize`) — published as GitHub Release assets. No Python install required.
- Add `release.yml` GitHub Actions workflow: tag-triggered, builds binaries on Windows + macOS arm64 + macOS x86_64, builds wheel/sdist on Linux, and publishes a GitHub Release with all artifacts attached.
